...
def run_attack(chat_id, target_ip, port, duration, method, threads, start_msg_id):
    ...
    try:
        ...
        print(f"[ATTACK STARTED] {target} for {duration}s | chat_id={chat_id} | msg_id={start_msg_id}")
        ...
        print(f"[ATTACK FINISHED] {target} - sending completion message...")
        
        # Try to edit the original message first
        edit_success = False
        if start_msg_id:
            # Small delay to ensure message exists
            time.sleep(1)
            edit_success = edit_telegram_message(chat_id, start_msg_id, completion_msg, 'HTML')
            if edit_success:
                print(f"[SUCCESS] Edited message {start_msg_id} for chat {chat_id}")
            else:
                print(f"[WARNING] Failed to edit message {start_msg_id}, sending new message instead")
        
        # If edit failed or no start_msg_id, send new message
        if not edit_success:
            send_success = send_telegram_message(chat_id, completion_msg, 'HTML')
            if send_success:
                print(f"[SUCCESS] Sent new completion message to chat {chat_id}")
            else:
                print(f"[ERROR] Failed to send completion message to chat {chat_id}")
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"[DEBUG] Attack thread starting for chat {chat_id} with msg_id {start_msg.message_id}")
    
    # Launch attack thread
    thread = threading.Thread(
        target=run_attack,
        args=(chat_id, target_ip, port, duration, "udp", 16, start_msg.message_id),
        daemon=True
    )
    thread.start()
...
def main():
    ...
    print("   - ✅ Automatic attack completion messages via direct API")
    print("   - 📝 All messages logged to console")
